<script lang="ts">
    ...
    let { onKeyPress, keyStates, enterPressed, disabled }: Props = $props();
    ...
    function getKeyStyle(key: string) {
        ...
        const baseStyle =
            "px-2 py-3 md:px-4 md:py-4 rounded border-2 font-bold text-xs md:text-sm transition-all duration-200 min-w-[32px] md:min-w-[40px] active:scale-95";
...
<div
    class="w-full max-w-2xl mx-auto space-y-1.5 md:space-y-2 -mt-1 md:mt-0 animate-[fadeIn_200ms_ease-out] px-[0px] py-[45px]"
>
    {#each rows as row}
        <div class="flex justify-center gap-1 md:gap-1.5">
            {#each row as key}
                <button
                    onclick={() => onKeyPress(key)}
                    {disabled}
                    class={getKeyStyle(key)}
                    style:min-width={key === "ENTER" || key === "BACKSPACE"
                        ? "60px"
                        : undefined}
                >
                    {key === "BACKSPACE" ? "←" : key}
