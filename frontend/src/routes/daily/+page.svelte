<script lang="ts">
    ...
    import DecryptionPanel from "./components/DecryptionPanel.svelte";
    import OutcomeMessage from "./components/OutcomeMessage.svelte";
    import SessionTimer from "./components/SessionTimer.svelte";
    import ClearancePanel from "./components/ClearancePanel.svelte";
    ...
    // User progression state — seeded from backend on mount
            ...
            if (res.ok) {
                ...
                data.guesses.forEach((g: any, i: number) => {
                    if (i < 6) {
                        ...
                        const states = g.colors.map((c: number) =>
                            c === 2
                                ? "correct"
                                : c === 1
                                  ? "present"
    ...
    function handleGameEnd(results: any) {
        ...
        // Update user total points from backend delta
        totalPoints += results.points_delta;

        // Sync auth store with new total
        if (auth.user) {
            auth.setUser({ ...auth.user, points: totalPoints });
        }
    ...
    async function submitGuess() {
        ...
        try {
            ...
            // Animate tiles resolving sequentially using Backend Data
            let currentTile = 0;
            const revealInterval = setInterval(() => {
                if (currentTile < 5) {
                    ...
                    if (data.status !== "IN_PROGRESS") {
                        // Safe fallback if results payload is missing
    ...
    async function fetchUserProfile() {
        try {
            ...
            if (res.ok) {
                ...
                // Populate auth store so LeaderboardPanel can identify [YOU]
                auth.setUser({
...
<div class="min-h-screen text-cyan-400 font-mono flex flex-col">
    ...
    <div class="lg:hidden fixed bottom-0 left-0 right-0 z-40">
        {#if gameStatus === "playing"}
            ...
            <!-- Panel content -->
            <div
                class={`bg-slate-900/95 border-t border-cyan-900/40 overflow-y-auto transition-all duration-300 ${
